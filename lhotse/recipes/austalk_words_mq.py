...
def download_austalk_words_mq(
    ...
    corpus_dir = target_dir / "austalk_words_mq"
...
def prepare_austalk_words_mq(
    ...
    for split in splits:
        ...
        for wav_file in tqdm(wav_files):
            ...
            speaker = '_'.join(idx.split("_")[0] + idx.split("_")[1])
            language = items[-2]
        ...
        if output_dir is not None:
            supervision_set.to_file(
                output_dir / f"austalk_words_mq_supervisions_{split}.jsonl.gz"
            )
            recording_set.to_file(output_dir / f"austalk_words_mq_recordings_{split}.jsonl.gz")
